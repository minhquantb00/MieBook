...
namespace BookManagement.Application.Handle.Media
{
    public class ProcessImageQuery : QueryString
    {
        ...
        public int? MaxWidth
        {
            get { return Get<int?>("w").HasValue() ? int.Parse(Get<int?>("w")) : 0; }
            set { Set("w", value); }
        }

        public int? MaxHeight
        {
            get { return Get<int?>("h").HasValue() ? int.Parse(Get<int?>("h")) : 0; }
            set { Set("h", value); }
        }

        public int? MaxSize
        {
            get { return Get<int?>("size").HasValue() ? int.Parse(Get<int?>("size")) : 0; }
            set { Set("size", value); }
        }

        public int? Quality
        {
            get { return Get<int?>("q").HasValue() ? int.Parse(Get<int?>("q")) : 0; }
            set { Set("q", value); }
        }
        ...
        private string Get<T>(string name)
        {
            return base[name];
        }
        ...
        private void Set<T>(string name, T val)
        {
            if (val == null)
                ...
                base.Add(name, val.ToString(), true);
        }
